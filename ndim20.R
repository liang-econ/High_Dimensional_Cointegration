...
pl2<-matrix(0,nr=np1,nc=length(sn1))
...
pl2[i,j]<-sqrt(sum(c((matrix(pf1[[1]]$beta[l1][[1]]@x[1:(dm0*dm0)],dm0)-bb1))^2))

print(c( rk1[i,j] , pl1[i,j] ) )
print(c( rk2[i,j],rk3[i,j] , pl2[i,j] ) )

rm(l1,dy0,ly1,ldy0,rf1,pf1)
}
save(rk1,pl1,rk2,pl2,rk3,file=fn1);
}
save(rk1,pl1,rk2,pl2,rk3,file=fn1);
